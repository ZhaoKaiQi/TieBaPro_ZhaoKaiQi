...
<!-- 创建最外层容器 -->
  ...
  <input name="user_nc" type="text" value="哈哈" style="display:none;" />
    <input name="tiezi_id" type="text" value="1" style="display:none;" />
  <!-- 创建内层容器用来装一行一行的内容 -->
    ...
    <!-- 创建第一行容器并加入内容 -->
      ...
      <!-- 创建隐藏表情框 -->
        ...
        <div id="context" class="context_line3" onclick="change_context_border()">
          <textarea id="Ideatext" name="IdeaContentName" class="text2_class" ></textarea>
        </div>
      </div>
      <!-- 创建发表按钮 -->
      <div  class="publish">
        <input id="publish" type="submit" class="publish_button" value="发布回复" onmouseover="change_submit_color()" onmouseout="change_submit_back()" />
